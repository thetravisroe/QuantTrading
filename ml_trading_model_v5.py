...
def choose_threshold(probs: np.ndarray, y_true: np.ndarray, lo: float = 0.4, hi: float = 0.6):
...
def main():
    print(">>> Starting ML trading predictor v5...")

    parser = argparse.ArgumentParser(description="ML-based next-day direction predictor (multi-ticker, multi-model).")
    parser.add_argument(
        "-t", "--ticker",
        type=str,
        default="SPY",
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        choices=["auto", "rf", "gb", "hgb"],
        help="Model type: 'rf', 'gb', 'hgb', or 'auto' to select the best based on recent data. Default: auto"
    )
    parser.add_argument(
        "--start",
        type=str,
    ...
    try:
        ...
        # =============== MODEL SELECTION ===============
        candidate_models = ["rf", "gb", "hgb"] if model_choice == "auto" else [model_choice]
        ...
        for name in candidate_models:
            print(f">>> Training {name.upper()} model on diagnostic split...")
            model = build_model(name)
            model.fit(X_train_dbg, y_train_dbg)
            probs_dbg = model.predict_proba(X_test_dbg)[:, 1]

            thr, tuned_acc = choose_threshold(probs_dbg, y_test_dbg)
            plain_preds = (probs_dbg >= 0.5).astype(int)
            plain_acc = accuracy_score(y_test_dbg, plain_preds)

            print(f"    {name.upper()} tuned threshold: {thr:.3f}, tuned acc: {tuned_acc:.3f}, acc@0.5: {plain_acc:.3f}")

            if tuned_acc > best_acc:
                best_acc = tuned_acc
                best_thr = thr
                best_plain_acc = plain_acc
                best_model_name = name
                best_model = model

        print(">>> Model selection result:")
        print(f"    Best model        : {best_model_name.upper()}")
        print(f"    Best tuned acc    : {best_acc:.3f}")
        print(f"    Best threshold    : {best_thr:.3f}")
        print(f"    Acc at 0.5 (best) : {best_plain_acc:.3f}")

        # =============== FINAL TRAIN ON FULL HISTORY (EXCEPT LAST DAY) ===============
        print(">>> Training best model on full history (except last day)...")
        X_hist = X.iloc[:-1]
        y_hist = y.iloc[:-1]
        X_live = X.iloc[[-1]]

        best_model.fit(X_hist, y_hist)
        ...
        prob_up = float(best_model.predict_proba(X_live)[0, 1])
